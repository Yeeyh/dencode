...
import com.tools.bp.crypt.Fence;

@Component
public class FencePanel extends BpPanel {
	private static final long serialVersionUID = 1L;
	public JTextArea area;
	public JCheckBox decode;
	public void init() {
		JLabel lurl = new JLabel("fence");
		area = new JTextArea(5,1);
		JScrollPane scroll = new JScrollPane(area);
		scroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);
		decode = new JCheckBox("removeSpace");
	...
	public void dencode(String text) {
		String result = "";
		Map<Integer, String> map = Fence.dencode(text,decode.isSelected());
		for(Entry<Integer,String> e : map.entrySet()) {
			Integer key = e.getKey();
			String value = e.getValue();
			result += "fence: "+key+";   "+value+"\n";
		}
		area.setText(result);
	}
